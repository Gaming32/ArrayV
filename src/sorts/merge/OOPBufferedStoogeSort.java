 ...
 * @author Gaming32
 */
public class OOPBufferedStoogeSort extends Sort {
    int[] aux;

    public OOPBufferedStoogeSort(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("OOP Buffered Stooge");
	...
	protected void mergeOOP(int[] arr, int start, int mid, int end) {
		...
		while (right > left && left >= start) {
			if (Reads.compareValues(aux[auxPtr], arr[left]) < 0) {
