...
SortPrompt.setSortThreadForCategory('Insertion Sorts', 14) {
    ...
    run(ClassicTreeSort, numbers: 2048, delay: arrayv.arrayManager.containsShuffle(Shuffles.RANDOM) ? 1 : 5).go()
    run(TreeSort, numbers: 2048, delay: arrayv.arrayManager.containsShuffle(Shuffles.RANDOM) ? 1 : 5).go()
